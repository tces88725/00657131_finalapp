...
struct FlightList: View {
    ...
    func fetchData() {
     let appId = "741fef3aad9d4ff89b0c5a006cd7ca0f"
        ...
        """
        let url = URL(string: "https://ptx.transportdata.tw/MOTC/v2/Air/FIDS/Airport/Departure/TPE?$top=30&$format=JSON")!
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data, let cResults = try? decoder.decode([Flight].self, from: data) {
              self.flights.append(contentsOf: cResults)
            }
        }.resume()
    }
    var body: some View {
        NavigationView {
            List(flights.indices, id: \.self) { (index) in
                NavigationLink(destination: FlightDetail(flight: self.flights[index])) {
                    FlightRow(flight: self.flights[index])}
            }.onAppear{
                if self.flights.count == 0{
                    self.fetchData()
                }
            }
        .navigationBarTitle("桃園中正國際機場")
            }
        }
