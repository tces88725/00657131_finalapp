...
struct hualienList: View {
    @State private var flights3 = [Flight]()
    ...
    func fetchData() {
     let appId = "741fef3aad9d4ff89b0c5a006cd7ca0f"
        ...
        """
        let url = URL(string: "https://ptx.transportdata.tw/MOTC/v2/Air/FIDS/Airport/Departure/HUN?$top=20&$format=JSON")!
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data, let cResults = try? decoder.decode([Flight].self, from: data) {
              self.flights3.append(contentsOf: cResults)
            }
        }.resume()
    }
    var body: some View {
        NavigationView {
            List(flights3.indices, id: \.self) { (index) in
                NavigationLink(destination: FlightDetail(flight: self.flights3[index])) {
                    FlightRow(flight: self.flights3[index])}
            }.onAppear{
                if self.flights3.count == 0{
                    self.fetchData()
                }
            }
            .navigationBarTitle("花蓮航空站")
            }
        }
}


struct hualienList_Previews: PreviewProvider {
