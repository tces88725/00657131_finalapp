...
struct kaohsiungList: View {
    @State private var flights11 = [Flight]()
    ...
    func fetchData() {
     let appId = "741fef3aad9d4ff89b0c5a006cd7ca0f"
        ...
        """
        let url = URL(string: "https://ptx.transportdata.tw/MOTC/v2/Air/FIDS/Airport/Departure/KHH?$top=20&$format=JSON")!
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data, let cResults = try? decoder.decode([Flight].self, from: data) {
              self.flights11.append(contentsOf: cResults)
            }
        }.resume()
    }
    var body: some View {
        NavigationView {
            List(flights11.indices, id: \.self) { (index) in
                NavigationLink(destination: FlightDetail(flight: self.flights11[index])) {
                    FlightRow(flight: self.flights11[index])}
            }.onAppear{
                if self.flights11.count == 0{
                    self.fetchData()
                }
            }
            .navigationBarTitle("高雄國際機場")
            }
        }
}
