...
struct matsuList: View {
    @State private var flights4 = [Flight]()
    ...
    func fetchData() {
     let appId = "741fef3aad9d4ff89b0c5a006cd7ca0f"
        ...
        """
        let url = URL(string: "https://ptx.transportdata.tw/MOTC/v2/Air/FIDS/Airport/Departure/MFK?$top=20&$format=JSON")!
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data, let cResults = try? decoder.decode([Flight].self, from: data) {
              self.flights4.append(contentsOf: cResults)
            }
        }.resume()
    }
    var body: some View {
        NavigationView {
            List(flights4.indices, id: \.self) { (index) in
                NavigationLink(destination: FlightDetail(flight: self.flights4[index])) {
                    FlightRow(flight: self.flights4[index])}
            }.onAppear{
                if self.flights4.count == 0{
                    self.fetchData()
                }
            }
            .navigationBarTitle("馬祖航空站")
            }
        }
}


struct matsuList_Previews: PreviewProvider {
