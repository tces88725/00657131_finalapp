...
struct pingtungList: View {
    @State private var flights9 = [Flight]()
    ...
    func fetchData() {
     let appId = "741fef3aad9d4ff89b0c5a006cd7ca0f"
        ...
        """
        let url = URL(string: "https://ptx.transportdata.tw/MOTC/v2/Air/FIDS/Airport/Departure/PIF?$top=20&$format=JSON")!
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data, let cResults = try? decoder.decode([Flight].self, from: data) {
              self.flights9.append(contentsOf: cResults)
            }
        }.resume()
    }
    var body: some View {
        NavigationView {
            List(flights9.indices, id: \.self) { (index) in
                NavigationLink(destination: FlightDetail(flight: self.flights9[index])) {
                    FlightRow(flight: self.flights9[index])}
            }.onAppear{
                if self.flights9.count == 0{
                    self.fetchData()
                }
            }
             .navigationBarTitle("屏東航空站")
            }
        }
}


struct pingtungList_Previews: PreviewProvider {
