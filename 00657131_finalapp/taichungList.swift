...
struct taichungList: View {
    @State private var flights7 = [Flight]()
    ...
    func fetchData() {
     let appId = "741fef3aad9d4ff89b0c5a006cd7ca0f"
        ...
        """
        let url = URL(string: "https://ptx.transportdata.tw/MOTC/v2/Air/FIDS/Airport/Departure/TXG?$top=20&$format=JSON")!
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data, let cResults = try? decoder.decode([Flight].self, from: data) {
              self.flights7.append(contentsOf: cResults)
            }
        }.resume()
    }
    var body: some View {
        NavigationView {
            List(flights7.indices, id: \.self) { (index) in
                NavigationLink(destination: FlightDetail(flight: self.flights7[index])) {
                    FlightRow(flight: self.flights7[index])}
            }.onAppear{
                if self.flights7.count == 0{
                    self.fetchData()
                }
            }
            .navigationBarTitle("台中國際機場")
            }
        }
}


struct taichungList_Previews: PreviewProvider {
