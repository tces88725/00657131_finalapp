...
struct tainanList: View {
    @State private var flights6 = [Flight]()
    ...
    func fetchData() {
     let appId = "741fef3aad9d4ff89b0c5a006cd7ca0f"
        ...
        """
        let url = URL(string: "https://ptx.transportdata.tw/MOTC/v2/Air/FIDS/Airport/Departure/TNN?$top=20&$format=JSON")!
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data, let cResults = try? decoder.decode([Flight].self, from: data) {
              self.flights6.append(contentsOf: cResults)
            }
        }.resume()
    }
    var body: some View {
        NavigationView {
            List(flights6.indices, id: \.self) { (index) in
                NavigationLink(destination: FlightDetail(flight: self.flights6[index])) {
                    FlightRow(flight: self.flights6[index])}
            }.onAppear{
                if self.flights6.count == 0{
                    self.fetchData()
                }
            }
            .navigationBarTitle("台南航空站")
            }
        }
}


struct tainanList_Previews: PreviewProvider {
